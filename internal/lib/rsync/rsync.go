...
type Options struct {
	// From https://github.com/zloylos/grsync
...
func getClientStorageDest(j *relique_job.ReliqueJob, path string) string {
	return filepath.Clean(fmt.Sprintf("relique@%s:%s", j.Client.Address, path))
}

func GetRestoreTask(j *relique_job.ReliqueJob, path string) *Rsync {
	...
	remotePath := fmt.Sprintf("relique@%s:%s", j.Client.Address, restoreDestination)
	...
	return New(path, localRestorePath, remotePath, rsyncOptions)
}
...
func GetBackupTask(j *relique_job.ReliqueJob, path string) *Rsync {
	...
	return New(path, remoteClientStorageDest, jobStorageRoot, rsyncOptions)
}
