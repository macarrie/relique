...
type BackupItem struct {
	Job            relique_job.ReliqueJob `json:"job,omitempty"`
...
func (b *BackupItem) GetSignature() error {
	...
	job, err := relique_job.GetByUuid(b.JobUuid)
	if err != nil {
...
func (b *BackupItem) ComputeChecksum() error {
	...
	if os.IsNotExist(err) || filePath == "/dev/null" {
		b.Exists = false
		return nil
	}
	b.Permissions = info.Mode()
...
func getSourceDiffPath(b *BackupItem) (string, error) {
	...
	previousJob, err := relique_job.GetPreviousJob(b.Job)
	if err != nil {
